...
class SkipgramWord2vecOp : public OpKernel {
 ...
 private:
  ...
  void NextExample(int32* example, int32* label) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    while (true) {
      if (label_pos_ >= label_limit_) {
        ...
        if(sentence_index_ % 4 == 0) { // subject
          label_pos_ = std::max<int32>(0, sentence_index_);
          label_limit_ =
            std::min<int32>(kSentenceSize, sentence_index_ + 3);
        }
        else if( sentence_index_ % 4 == 1) { // property
          label_pos_ = std::max<int32>(0, sentence_index_ - 1);
          label_limit_ =
            std::min<int32>(kSentenceSize, sentence_index_ + 2);
        }
        else if(sentence_index_ % 4 == 2) { // object
          label_pos_ = std::max<int32>(0, sentence_index_ - 2);
          label_limit_ =
            std::min<int32>(kSentenceSize, sentence_index_ );
        }
        else{
          label_pos_ = std::max<int32>(0, sentence_index_ + 1);
          label_limit_ =
    ...
    if(sentence_index_  % 4 != 3){
      // LOG(INFO) << "  example " << sentence_[sentence_index_] 
      //         << "  label  " << sentence_[label_pos_]
      //         << "  sentence_index_, " << sentence_index_ 
      //         << "  label_pos_ " << label_pos_;
  ...
  Status Init(Env* env, const string& filename) {
    ...
    StringPiece input = ReplaceAll(data, std::string("\n"), std::string(" / "));
